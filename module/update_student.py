...
class DepthHeadStudent(nn.Module):
    def __init__(self, input_dim=16, hidden_dim=32, output_dim=1):
        super(DepthHeadStudent, self).__init__()
...
class ConvGRUStudent(nn.Module):
    def __init__(self, hidden_dim, input_dim, kernel_size=3):
        super(ConvGRUStudent, self).__init__()
...
class MotionEncoderStudent(nn.Module):
    def __init__(self, cor_planes, c1_planes=16, c2_planes=16, d1_planes=16, d2_planes=16, out_planes=32):
        super(MotionEncoderStudent, self).__init__()
...
class UpdateBlockStudent(nn.Module):
    def __init__(self, opts, hidden_dim, input_dim):
        ...
        self.encoder = MotionEncoderStudent(cor_planes)
        encoder_output_dim = 32

        self.gru = ConvGRUStudent(hidden_dim, encoder_output_dim + input_dim)
        self.depth_head = DepthHeadStudent(hidden_dim, hidden_dim=32, output_dim=1)

        factor = 2 ** self.opts.num_downsample
        self.mask = nn.Sequential(
            nn.Conv2d(hidden_dim, 32, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, (factor ** 2) * 9, 1, padding=0),
        )
