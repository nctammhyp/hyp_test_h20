...
def readImage(path: str, read_or_die=True):
    try:
        # Sử dụng IMREAD_UNCHANGED để giữ nguyên số kênh (quan trọng cho mask/gray)
        image = cv2.imread(path, cv2.IMREAD_UNCHANGED)
        if image is None:
            raise Exception(f"File not found or invalid: {path}")
        
        # Nếu là ảnh màu (3 kênh BGR), chuyển sang RGB
        if len(image.shape) == 3 and image.shape[2] == 3:
