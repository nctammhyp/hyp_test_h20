...
def train(opts, load_state, teacher_ckpt):
    ...
    student = nn.DataParallel(ROmniStereoStudent(opts.net_opts)).cuda()
    if opts.net_opts.fix_bn:
        student.module.freeze_bn()

    teacher = load_teacher(teacher_ckpt, opts.net_opts)
...
def main():
    parser = ArgumentParser(description="KD Training for Student (1/3 Teacher)")

    parser.add_argument("--name", default="ROmniStereo_Student", help="experiment name")
    ...
    parser.add_argument("--num_invdepth", type=int, default=16, help="number of disparity")
    parser.add_argument("--equirect_size", type=int, nargs="+", default=[128, 320], help="out ERP size")
    parser.add_argument("--use_rgb", action="store_true", help="use 3-channel rgb input")

    parser.add_argument("--base_channel", type=int, default=8, help="student base channel")
    ...
    train(opts, load_state, args.teacher_ckpt)
